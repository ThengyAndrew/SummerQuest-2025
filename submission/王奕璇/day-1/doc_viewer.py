...
class SimpleLark:
    ...
    def _get_req(self, url: str, headers: Optional[Dict[str, str]] = None, param: Optional[Dict[str, str]] = None) -> \
    Dict[str, Any]:
        if param is not None:
            url = url + '?' + urlencode(param)
        try:
        ...
        if field_name and field_value:
            ...
            for record in all_records:
                ...
                if isinstance(field_val, str) and field_value in field_val:
                    # print(f"field_val is str")
                    filtered_records.append(record)
                elif str(field_val) == field_value:
                    filtered_records.append(record)
                elif isinstance(field_val, list):
                    # print(f"field_val is list")
                    # print(field_val)
                    for item in field_val:
                        if item["en_name"]==field_value:
                            filtered_records.append(record)
                elif isinstance(field_val, dict):
                    print("field_val is a dict!")
...
if __name__ == "__main__":
    # 从环境变量获取配置信息
    app_id = os.environ.get("FEISHU_APP_ID", "cli_a8e1f4486879900e")
    app_secret = os.environ.get("FEISHU_APP_SECRET", "6aaxPDETXlit9ZlfBFbhsguvzOAnAOSo")
    ...
    print("1. 获取所有记录:")
    try:
        lark.auth.get_token(force_refresh=True)
    ...
    field_values = ["刘智耿", "刘智耿"]  # 替换为你要筛选的值
